...
class DoubleClickDownUpLeftTrigger(TriggerBase):
    """
    触发条件：连续两次左键点击（双击）后，鼠标整体趋势下->上->左移动，每段只要位移趋势达标即可，不要求直线。
    参数：
        ...
        if self.gesture_stage > 0:
            ...
            if self.gesture_stage == 1:
                ...
                if dy < -self.min_move:
                    ...
                    self.ref_pos = (mouse_x, mouse_y)
            elif self.gesture_stage == 3:
                # 向左趋势，x轴减少即可
                if dx < -self.min_move:
                    print(">>> 手势3/3 向左趋势达成，全部手势完成，触发回调")
                    self.gesture_stage = 0
                    self.on_trigger()
            # 若逆向移动太多，重置
    ...
    def on_trigger(self):
        if self.callback:
            ...
            print("DoubleClickDownUpLeftTrigger: 触发事件")