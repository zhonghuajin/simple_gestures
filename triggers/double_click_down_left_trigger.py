...
class DoubleClickDownLeftTrigger(TriggerBase):
    """
    触发条件：连续两次左键点击（双击）后，鼠标整体趋势先向下再向左移动，
    只有在判定向左动作时，鼠标y坐标需在屏幕下半部分。
    参数：
    ...
    def update(self, state):
        ...
        if self.gesture_stage > 0:
            ...
            if self.gesture_stage == 1:
                ...
                # 向左趋势，x轴减少即可，且此时y坐标要在屏幕下半部分
                if dx < -self.min_move and mouse_y >= screen_height // 2:
                    print(">>> 手势2/2 向左趋势达成（且在屏幕下半部分），全部手势完成，触发回调")
                    self.gesture_stage = 0
                    self.on_trigger()
                elif dx < -self.min_move:
                    print(">>> 手势2/2 向左趋势达成，但未在屏幕下半部分，忽略")
                    self.gesture_stage = 0
            # 若逆向移动太多，重置
    ...
    def on_trigger(self):
        if self.callback:
            ...
            print("DoubleClickDownLeftTrigger: 触发事件")